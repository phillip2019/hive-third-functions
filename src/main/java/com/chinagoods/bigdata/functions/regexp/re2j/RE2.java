...
import static com.chinagoods.bigdata.functions.regexp.re2j.Options.Algorithm.DFA;
import static com.chinagoods.bigdata.functions.regexp.re2j.Options.Algorithm.DFA_FALLBACK_TO_NFA;
...
class RE2 {
    ...
    boolean match(Slice s) {
        return doExecute(MachineInput.fromUTF8(s), 0, Anchor.UNANCHORED, 0) != null;
    }
    ...
    // This is visible for testing.
    Slice find(Slice s) {
        int[] a = doExecute(MachineInput.fromUTF8(s), 0, Anchor.UNANCHORED, 2);
        if (a == null) {
    ...
    Slice[] findSubmatch(Slice s) {
        int[] a = doExecute(MachineInput.fromUTF8(s), 0, Anchor.UNANCHORED, prog.numCap);
        if (a == null) {
            return null;
        }
    ...
    int[] findSubmatchIndex(Slice s) {
        return pad(doExecute(MachineInput.fromUTF8(s), 0, Anchor.UNANCHORED, prog.numCap));
    }
