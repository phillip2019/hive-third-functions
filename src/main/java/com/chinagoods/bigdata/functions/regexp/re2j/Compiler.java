...
class Compiler {
    ...
    private Frag rune(int rune, int flags) {
        if ((flags & RE2.FOLD_CASE) == 0) {
            ...
            return alt(rune(new int[]{rune}, flags), rune(new int[]{Unicode.simpleFold(rune)}, flags));
        }
    }
    ...
    private void expandRuneRange(int lo, int hi, List<Integer> runes) {
        ...
        for (int i = 1; i < Unicode.UTF_MAX; i++) {
    ...
    private Frag rune(int lo, int hi, int flags) {
        return bytes(Unicode.codePointToUtf8(lo), Unicode.codePointToUtf8(hi), flags);
    }
