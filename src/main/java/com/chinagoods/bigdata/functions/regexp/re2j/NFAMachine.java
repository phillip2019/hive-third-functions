...
class NFAMachine implements Machine {
    ...
    public boolean match(MachineInput in, int pos, Anchor anchor, int[] submatches) {
        ...
        if (pos == 0) {
            flag = Utils.emptyOpContext(MachineInput.EOF, b);
        } else {
            flag = Utils.emptyOpContext(in.getByte(pos - 1), b);
        }
        for (; ; ) {
            ...
            if (!matched && (pos == 0 || anchor.isUnanchored()) && (b == MachineInput.EOF || Utils.isRuneStart(b))) {
